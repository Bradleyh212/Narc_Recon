def open_receiving():
	...
	header_frame = tk.Frame(receiving_window, width = w, height = 100, bg = nav_background_color) # using the bg to see the frames
	...
	def refresh_page():
		...
		page_title = tk.Label(header_frame, text = "RECEIVING", fg = "white", bg = nav_background_color, font = header_font)
		page_title.grid(row = 0, column = 0, sticky = "w", padx = 30)

		home_btn = ttk.Button(nav_frame, text = "HOME", style='TButton', command = lambda : [receiving_window.destroy(), open_main_page()])
		home_btn.grid(row = 0, column = 0, padx = 6, pady = 40) #Used the lambda key word to use 2 functions in 1 button

		receiving_btn = ttk.Button(nav_frame, text = "RECEIVING", style='TButton')
		...
		reconciliation_btn = ttk.Button(nav_frame, text = "RECONCILIATION", style='TButton', command = lambda : [receiving_window.destroy(), open_reconciliation_page()])
		...
		report_btn = ttk.Button(nav_frame, text = "REPORT", style='TButton', command = lambda : [receiving_window.destroy(), open_report_page()])
		report_btn.grid(row = 0, column = 3, padx = 3)

		log_off_btn = ttk.Button(nav_frame, text = "LOGOUT", style='TButton', command = lambda : [receiving_window.destroy()]) 
		...
		add_btn = ttk.Button(right_body_frame, text = "ADD QUANTITY", style='TButton', command=lambda: add_quantity(add_qty_ent.get(), search_input))
		add_btn.grid(row = 4)
