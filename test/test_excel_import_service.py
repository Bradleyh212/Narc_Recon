...
def test_excel_import_service_validate_columns_missing_required_column_raises():
	df = sample_excel_df().drop(columns=["Upc"])
	service = excel_import_service.ExcelImportService()
...
def test_excel_import_service_validate_rows_reports_current_warning_shape():
	df = pd.DataFrame([
		{
			"DIN": 2248809,
			"Drug Name": "ADDERALL XR",
	...
	report = service.validate_rows(df)
...
def test_excel_import_service_create_narc_list_preserves_shape(tmp_path):
	excel_path = tmp_path / "med_sheet.xlsx"
	sample_excel_df().to_excel(excel_path, sheet_name="med_sheet", index=False)
	service = excel_import_service.ExcelImportService(excel_path)

	narc_list = service.create_narc_list()
...
def test_excel_import_service_import_to_sql_uses_insert_or_ignore_behavior():
	conn = sqlite3.connect(":memory:")
	cursor = conn.cursor()
	cursor.execute("""
		CREATE TABLE narcs (
	...
	service.import_to_sql(cursor, narc_list)
	service.import_to_sql(cursor, narc_list)
