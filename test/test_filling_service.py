...
def test_decrement_inventory_quantity_updates_quantity_and_commits():
	...
	filling_service.decrement_inventory_quantity(cursor, connection, "02248809", 3)

	assert connection.execute("SELECT quantity FROM narcs WHERE din = ?", ("02248809",)).fetchone()[0] == 7


def test_decrement_inventory_quantity_preserves_current_float_amount_behavior():
	connection = make_inventory_connection(10)
	cursor = connection.cursor()

	filling_service.decrement_inventory_quantity(cursor, connection, "02248809", 2.5)
